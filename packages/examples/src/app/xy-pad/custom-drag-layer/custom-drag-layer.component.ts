...
@Component({
  ...
  <ng-container *ngIf="c.isDragging">
    ...
    <app-crosshairs *ngIf="crossStyle$|async as cross"
      [x]="cross.x"
      [y]="cross.y">
    </app-crosshairs>
    ...
    <div [ngStyle]="movingStyle$|async">
      <ng-container [ngSwitch]="c.itemType">
        <app-box-drag-preview *ngSwitchCase="'BOX'"
          [title]="c.item.title">
...
export class CustomDragLayerComponent {
  ...
  @Input() set incrementPx(n: number) {
  ...
  getXY(spot: Spot, initialOffset: Offset, currentOffset: Offset, emit = false): Offset {
    ...
    if (spot.fromCube) {
      let absoluteUsingOriginalSpot = plus(spot, minus(diff, { x: 16, y: 16 }));
